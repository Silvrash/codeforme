...
class DIConfigManager(dict):
    ...
    def __init__(self, settings_dict, **kwargs):
...
class DIContainer(object):
    ...
    def __init__(self, settings, *args, **kwargs):
        ...
        # _logger.debug('Container __init__ called. Begin to bootstrap this container.')
        ...
        for key, conf in self.settings.items():
            if not conf.lazy:
                # _logger.debug('found non-lazy configuration %s. resovling it.', key)
                self.resolve(key)
        ...
        self.debug = kwargs.get('debug', False) # type: bool
    ...
    def register(self, name, settings=None, replace=False):
        ...
        if name in self.settings:
            if replace:
                if self.debug:
                    _logger.debug("name %s is already registerd. will be replaced.", name)
                pass
            else:
        ...
        try:
            ...
            for key, conf in settings_iter():
                if name in conf.alias:
                    # _logger.debug("%s could not be found. found it as alias for %s.", name, key)
                    name = key
                    break
            else:
    ...
    def resolve_type(self, name):
        ...
        try:
            ...
            # if there is a parent given, check if there is a configuration
    ...
    def build_up(self, name, instance, **overrides):
        """
        Injects the information specified in the properties config
        into an existing object.

        :param name: name of the object definition in the container config.
        :type name: str
        :param instance: the instance to build up
    ...
    def clear(self, name=None):
        ...
        :param name: the name of the singleton instance that should be
                     destroyed.
        :type name: str
        """
        if name is not None:
    ...
    def _inject(self, resolve_method, force=False, **inject_kwargs):
        def wrapper(func):
            ...
            def inner(*args, **kwargs):
                ...
                # _logger.debug('calling decorated function %s with %s.', func.__name__, kwargs)
                return func(**kwargs)
